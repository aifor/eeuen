...
def model(is_treat, embeddings, label, is_training=True):
    with tf.variable_scope('Dragonnet', reuse=True) as scope:
        ...
        net = DRAGONNET(input, is_treat, label, 64,
                        False, 0.01, 1., True, "elu", is_training=is_training)
...
def train_xdl_io():
    ...
    with xdl.model_scope('train1'):
        losses1, _ = model(is_treat, fea_list, labels, True)
    ...
    with xdl.model_scope('train2'):
        losses2, _ = model(is_treat, fea_list, labels, True)
